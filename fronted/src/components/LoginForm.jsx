...
function LoginForm() {
  ...
  const loginHandler = async (data) => {
    try {
      ...
      dispatch(login({
        userData: result.resUser,
        accessToken: result.accessToken
      }));
      console.log("rech final");
      navigate("/");
    } catch (error) {
...
<h1 style={{paddingBottom:"10px", textAlign:"center" }}><b>Login form</b></h1>
      ...
      <form onSubmit={handleSubmit(loginHandler)} className="space-y-4">
        ...
        <Link to="/signupForm" style={{color:"blue", paddingTop:"5px"}}>If you have not register you can registern from here first</Link>
      </form>
    </div>
  );
}
